...
export async function POST(request: NextRequest) {
  console.log("[v0] Received POST request to /api/weather")
  
  try {
    // 1. Validar API Key
    console.log("[v0] Step 1: Validating API Key")
    ...
    if (!apiKey) {
      console.log("[v0] Missing API key")
    ...
    try {
      ...
      console.log("[v0] Body parsed successfully:", body)
    } catch (parseError) {
    ...
    if (!WeatherValidator.validatePayload(body)) {
      console.log("[v0] Invalid payload format:", body)
      return NextResponse.json(
    ...
    // 3. Validar rangos
    console.log("[v0] Step 3: Validating value ranges")
    const { valid, errors } = WeatherValidator.validateRanges(body)
    if (!valid) {
      console.log("[v0] Values out of range:", errors)
      return NextResponse.json({ error: "Values out of range", details: errors }, { status: 400 })
    }

    // 4. Insertar lectura en la base de datos
    console.log("[v0] Step 4: Inserting reading to database")
    const reading = await WeatherRepository.insertReading(body)
    if (!reading) {
      console.error("[v0] Failed to insert reading")
      return NextResponse.json({ error: "Failed to save reading" }, { status: 500 })
    }
    console.log("[v0] Reading inserted successfully:", reading.id)

    // 5. Actualizar extremos diarios (no bloqueante)
    console.log("[v0] Step 5: Starting background task to update daily extremes")
    WeatherRepository.updateDailyExtremes(reading).catch((error) => {
      console.error("[v0] Background task error updating extremes:", error)
    })
    ...
    console.error("[v0] Unhandled error in weather API:", error)
